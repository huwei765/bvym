...
function get_con_name($id){
    $data=M('custcon')->where('id='.$id)->getField('xingming');
    return $data;
}
...
function getSex($sex_code){
    if($sex_code == "1"){
        return iconv('GB2312', 'UTF-8', "男");
    }
    elseif($sex_code == "2"){
        return iconv('GB2312', 'UTF-8', "女");
    }
    else{
        return iconv('GB2312', 'UTF-8', "无");
    }
}
 ...
 * @return string
 */
function getCustFenlei($fenlei){
    $fenlei_name = "";
    switch($fenlei){
        case "0":
            $fenlei_name = iconv('GB2312', 'UTF-8', "有意向");
            break;
        case "1":
            $fenlei_name = iconv('GB2312', 'UTF-8', "洽谈");
            break;
        case "2":
 ...
 * @return string
 */
function getCustCate($type){
    $type_name = "";
    switch($type){
        case "0":
            $type_name = iconv('GB2312', 'UTF-8', "美容院");
            break;
        case "1":
            $type_name = iconv('GB2312', 'UTF-8', "代理商分公司");
            break;
    }
    return $type_name;
}
...
function getCustType($type){
    ...
    switch($type){
        case "0":
            $type_name = iconv('GB2312', 'UTF-8', "上门拜访");
            break;
        case "1":
 ...
 * @return string
 */
function getCustHtType($type){
    $type_name = "";
    switch($type){
        case "0":
            $type_name = iconv('GB2312', 'UTF-8', "劳务合同");
            break;
        case "1":
            $type_name = iconv('GB2312', 'UTF-8', "其它");
            break;
    }
    return $type_name;
}
...
function getPayWay($type){
    ...
    switch($type){
        case "0":
            $type_name = iconv('GB2312', 'UTF-8', "现金");
            break;
        case "1":
...
/**
 * 从config文件中通过val值获取key值
 * @param $value
 * @param $arr
 * @return int|string
 */
function getConfigKeyByVal($value,$arr){
    $key_name = "";
    if(is_array($arr)){
        foreach($arr as $item){
            if(is_int($value)){
                if(intval($item["value"]) == $value){
                    $key_name = $item["name"];
                    break;
                }
            }
